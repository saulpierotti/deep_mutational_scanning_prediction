...
plot <-   base_plot +
  ...
  scale_color_manual(
    name = NULL,
    labels = NULL,
    values = c(okabe[1], "black"),
    guide = NULL
  ) +
  theme(legend.position = "none")

presentation_plot <- base_plot +
  geom_jitter(
    data = curr_tb %>% filter(aa1 == aa2),
    mapping = aes(color = bologna_red),
    shape = 4,
    alpha = 0.2,
    height = 0.1,
    width = 0
...
presentation_plot <- base_plot +
    scale_fill_manual(
    values = c(barplot_gray, bologna_red),
    name = NULL,
    labels = c("Missense", "Synonymous")
  ) +
  theme(legend.position = "top")
...
base_plot <- ggplot(mapping = aes(x = Ubiquitin, y = E1_Ubiquitin)) +
  geom_point(
