...
```{r single protein naive performance confidence intervals}
get_corr_bootstrap(test_result_naive_tb, method = "pearson", n_reps = 10000)
```
...
plot <-
  get_performance_comparison_plot(
    ...
    mutate(model = fct_relevel(model, c("by_position", "naive", "LOPO")))
    ) +
