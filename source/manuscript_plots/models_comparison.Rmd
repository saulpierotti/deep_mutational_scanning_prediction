...
plot <-
  get_performance_comparison_plot(
    single_protein_comparison_tb %>%
      filter(method == "pearson")) +
  ylab("Pearson correlation coefficient")

plot_presentation <-  get_performance_comparison_plot(
  single_protein_comparison_tb %>%
    filter(method == "pearson") %>%
    mutate(model=fct_relevel(model,"naive")),
  use_outline = TRUE,
  size = 1.3,
  dodge_width = 0
) +
...
#             "presentation_single_protein_models_performance_comparison.tex",
...
```{r xgb models performance comparison plot, dev="tikz", results="hide"}
plot <-
  get_performance_comparison_plot(
