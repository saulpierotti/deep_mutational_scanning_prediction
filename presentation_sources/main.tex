...
\begin{frame}
	...
	%	\begin{tikzpicture}
	%		\tikzstyle{every node}=[object, font=\footnotesize]
	%		\node [object, minimum height=4em, text width=15em] (a) {Using residue contacts in a graph convolutional neural network};
	%		\node [object, minimum height=4em, text width=15em, right=1em of a] (b) {Unsupervised models seem promising and may be worth exploring more};
	%		\node [object, minimum height=4em, text width=15em, below=2em of a] (c) {Training on more deep mutational scanning studies};
	%		\node [object, minimum height=4em, text width=15em, right=1em of c] (d) {Finding a better normalization strategy for the scores from different experiments};
	%		\node [object, minimum height=4em, text width=12em, below=2em of $(c.south)!0.5!(d.south)$] (e) {Tuning the set of features};
	%	\end{tikzpicture}
	%\end{figure}
\end{frame}
