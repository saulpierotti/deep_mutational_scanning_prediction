...
\cref{tab:datasets} reports a summary of each dataset, while \cref{tab:excluded} reports the number of mutations in each of them before and after filtering.
...
\begin{figure}[p]
	\ffigbox[\textwidth]{%
		...
		\end{subfloatrow}%
			...
			The same data presented individually for each dataset is available in appendix (\cref{sup:fig:aa2_frequencies_by_dataset.tex}, \cref{sup:fig:dataset_composition_by_dataset}, and \cref{sup:fig:median_residue_fitness_by_dataset}).
...
Numerical quantification of the strength of the relationship between each of the features and the fitness scores is reported in \cref{tab:features_score_relationship}.
For the features that are themselves predicted quantities, the agreement between them and their experimental counterpart is reported in \cref{tab:netsurf_validation} and \cref{tab:trrosetta_validation}.
...
\cref{subfig:dssp_q3} reports the aggregated distribution of the median fitness scores (per dataset and position) by secondary structure.
\cref{sup:fig:score_dssp_q3_by_dataset} reports the same but detailed by dataset.
...
The predicted secondary structures from NetSurfP-2 are in good agreement with the experimentally determined ones, where the latter are available (\cref{tab:netsurf_validation}).

\subsection{Torsion angles}
The relationship between backbone torsion angles and fitness scores is negligible (\cref{tab:features_score_relationship}).
...
The single protein models trained by segregating protein positions in different splits for validation and testing performed sensibly worse than the ones trained with the naive approach (\cref{subfig:single_protein_models_performance_comparison}).
...
\cref{fig:performance_comparison} shows the performance of two state-of-the-art mutation effect predictors compared to that of the general models developed as part of this work.
...
For a comparison of the validation and test scores of my models, refer to \cref{sup:fig:test_summary}.
...
\begin{figure}[p]
	\ffigbox[\textwidth]{%
		...
		\label{fig:learning_curves_single_proteins}%
	}%
\end{figure}
